...
BACKUP_NAME="strapi-cloud-backup-${TIMESTAMP}"
...
cleanup_old_backups() {
    log_info "Cleaning up old cloud backups (keeping last 10)..."
    cd "$BACKUP_DIR"
    ls -t strapi-cloud-backup-*.tar.gz 2>/dev/null | tail -n +11 | xargs -r rm -f
    cd - > /dev/null
...
main() {
    ...
    ls -la "$BACKUP_DIR"/strapi-cloud-backup-*.tar.gz 2>/dev/null | tail -5 || log_warning "No previous cloud backups found"
}
