...
create_backup_before_deploy() {
    ...
    if pnpm strapi export --file "${LOCAL_BACKUP_DIR}/pre-deploy-${TIMESTAMP}" --no-encrypt > /dev/null 2>&1; then
        log_success "Pre-deployment backup created: pre-deploy-${TIMESTAMP}.tar.gz"
    else
        log_warning "Failed to create pre-deployment backup"
    fi
}
...
main() {
    ...
    log_info "Local backup: ${LOCAL_BACKUP_DIR}/pre-deploy-${TIMESTAMP}.tar.gz"
    log_info "Cloud backup: ./backups/cloud/strapi-cloud-backup-*.tar.gz"
